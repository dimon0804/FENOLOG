...
Второй набор оказался тяжелее первого, и причина объективна: Sentinel-2 в этих
данных начинается только с 2017 года, а во втором тесте 24,8 % контрольных точек
приходится на более ранние годы против 18,2 % в первом. До 2017-го наблюдений
вдвое меньше, и они с более шумных Landsat и MODIS. Разбор — E22 в журнале
экспериментов.
...
ESA WorldCereal и WorldCover. Мы используем только OpenStreetMap и говорим об
этом прямо. Причина: WorldCereal отдаёт растровую маску пахотных земель, а не
границы конкретных участков, и чтобы получить из неё контур поля, нужен отдельный
шаг векторизации и разделения смежных полей — работа, сопоставимая по объёму со
всем слоем сбора данных. OpenStreetMap отдаёт готовые замкнутые контуры с
идентификатором, по которому поле узнаётся при повторном заходе. Для степной зоны
Юга России покрытие OSM неполное, и это записано в границах применимости.
