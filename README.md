...
| Второй тур (2 323 точки) | — | **RMSE 0,0749** |
...
Результат воспроизводится **побитово**: два запуска дают файл с точностью до
последнего знака. Готовая модель подхватывается из `models/`, переобучение не
требуется — весь прогон около полутора минут.
