...
def attribute_cause(
    ...
    if np.isfinite(ratio) and np.isfinite(temp_anom):
        ...
        if tight:
            # Погода действительно в норме по обоим показателям — версию можно
            # формулировать утвердительно.
            text = (
                ...
                "проверяйте агротехнику, состояние семян, болезни и вредителей."
            ).format(precip_actual, precip_norm, ratio * 100, temp_anom, duration)
        else:
            # Пограничный случай: ни один погодный порог не сработал, но какой-то
            # показатель заметно отклонён. Утверждать «погоды тут нет» нельзя —
            # раньше сервис в одной фразе называл 55 процентов от нормы осадков и
            # тут же заявлял, что засухи не было. Такое противоречие обесценивает
            # весь вывод, поэтому пограничный случай называется пограничным.
            hints = []
            if ratio < NON_WEATHER_PRECIP_LO:
                hints.append("осадков заметно меньше нормы")
            elif ratio > NON_WEATHER_PRECIP_HI:
                hints.append("осадков заметно больше нормы")
            if abs(temp_anom) > NON_WEATHER_TEMP_C:
                hints.append("температура отклонена от нормы на {:.1f} градуса".format(temp_anom))
            hint = ", ".join(hints) if hints else "погода отклонена умеренно"
            text = (
                "Осадков {:.0f} мм при норме {:.0f} мм ({:.0f} процентов), температура "
                "отличается от нормы на {:.1f} градуса. Ни один погодный порог не "
                "превышен, хотя {}. Погода объясняет просадку лишь отчасти: "
                "проверяйте агротехнику, состояние семян, болезни и вредителей."
            ).format(precip_actual, precip_norm, ratio * 100, temp_anom, hint)

        return (CAUSE_NON_WEATHER, 0.5 if tight else 0.35, evidence, text)
