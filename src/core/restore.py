...
Замеры на локальной валидации (3495 спрятанных точек):
    среднее двух соседей   RMSE 0.0888
    линейная интерполяция  RMSE 0.0880
    Уиттекер, lam=1000     RMSE 0.0794
    смесь 50/50            RMSE 0.0834  <- конфигурация ядра

    Числа по действующему протоколу валидации. Прежние (0.0907 / 0.0894 / 0.0875 /
    0.0849) взяты из черновой версии протокола, где геометрия скрытых точек не
    совпадала с настоящей; они недействительны и в отчёт не переносятся.
"""
